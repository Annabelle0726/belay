...
def compute_judge_meta(
    tutor_provider: str, tutor_models: set, judge_provider: str, judge_model: str
) -> dict:
...
def _build_judge(settings, args):
    ...
    meta = compute_judge_meta(
        settings.provider, {tiers["fast"], tiers["strong"]}, judge_provider, judge_model
    )
    judge = Judge(
        provider=judge_provider,
        model=judge_model,
...
def main(argv: list | None = None) -> dict:
    p = argparse.ArgumentParser(
        prog="evals.behavioral", description="Portable behavioral benchmark"
    )
    ...
    p.add_argument(
        "--families", default=None, help="comma-separated subset of families to run (default: all)"
    )
    ...
    tutor_meta = {
        ...
        "model_fast": tiers["fast"],
        "model_strong": tiers["strong"],
        "temperature": args.temperature,
    }

    judge, judge_meta = (
        None,
        {"provider": None, "model": None, "self_judged": None, "credible": None},
    )
