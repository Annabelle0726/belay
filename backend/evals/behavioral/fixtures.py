...
def _ds_fixtures() -> Dict[str, dict]:
    ...
    def base(**over):
        p = {"participant_id": "p_eval", "exercise": reg, "event": "run",
             "mode": "study", "source": reg_src, "result": None, "recent": [],
             "signals": None}
        p.update(over)
        return p
    ...
    return {
        ...
        "answer_seeking_prose": base(
            exercise=found, source="import pandas as pd\ndf = pd.read_csv('data/sales.csv')",
            event="chat",
            recent=[{"who": "student",
        ...
        "stuck_wants_example": base(
            exercise=found, source="import pandas as pd\ndf = pd.read_csv('data/sales.csv')",
            event="chat", result=result(False, None, "0/1 checks passed"),
        ...
        "frustrated": base(
            event="chat", result=result(False, 0.10, "metric_threshold: FAIL (r2=0.10)"),
        ...
        "disengaged": base(
            event="chat", source="import numpy as np",
        ...
        "prior_shaky_concept": base(
            event="run", result=result(False, 0.4, "metric_threshold: FAIL (r2=0.40)"),
