...
def test_set_update_clear_goals():
    ...
    assert goals_mod.get_goals(store.get_learner_state("p1"))["text"].startswith("I want to explain")
...
def test_goals_survive_a_turn():
    ...
    assert goals_mod.get_goals(store.get_learner_state("p_goal"))["text"] == "explain reasoning first"
