...
class StubLLM:
    ...
    def __init__(
        self,
        leak=False,
        planner_intervention="co_reason",
        eval_confidence=0.75,
        planner_confidence=0.70,
        reasoner_confidence=0.80,
    ):
        self.leak = leak
    ...
    def json(self, *, role, tier, system, user, max_tokens=800, reasoning_effort=None):
        ...
        if role == "self_eval":
            ...
            return {
                ...
                "self_critique": "looks reasonable",
                "reasons": [],
            }
...
class TestPeerStance:
    ...
    def test_escalate_flags_for_instructor(self):
        ...
        out = run_turn(
            _payload("p7", "peer"), StubLLM(planner_intervention="escalate"), InMemoryStore()
        )
    ...
    def test_answer_seeking_is_redirected(self):
        out = run_turn(
            _payload("p8", "peer", recent=_ANSWER_SEEKING), StubLLM(leak=False), InMemoryStore()
        )
...
class TestOracleStance:
    ...
    def test_answer_seeking_not_redirected(self):
        ...
        out = run_turn(
            _payload("o7", "oracle", recent=_ANSWER_SEEKING), StubLLM(leak=False), InMemoryStore()
        )
    ...
    def test_planner_never_escalates_or_reciprocates(self):
        ...
        for forced in ("escalate", "reciprocate"):
            out = run_turn(
                _payload("o8", "oracle"), StubLLM(planner_intervention=forced), InMemoryStore()
            )
...
class TestEscalation:
    ...
    def test_low_confidence_escalates_at_higher_effort(self):
        ...
        for stance in ("peer", "oracle"):
            ...
            assert (
                out["components"]["reasoning_effort"] == settings.reasoner_effort_escalated
            ), stance
...
class TestAbstention:
    def test_peer_abstains_below_floor(self):
        out = run_turn(
            _payload("ab_peer", "peer"), StubLLM(eval_confidence=_CONF_FLOOR), InMemoryStore()
        )
    ...
    def test_control_never_escalates_or_abstains(self):
        out = run_turn(
            _payload("ab_ctrl", "control"), StubLLM(eval_confidence=_CONF_FLOOR), InMemoryStore()
        )
...
class TestConfidenceTrajectory:
    def test_trajectory_present_both_arms(self):
        for stance in ("peer", "oracle"):
            ...
            assert (
                row["payload"]["telemetry"]["confidence_trajectory"]["self_eval"]
                == traj["self_eval"]
            )
