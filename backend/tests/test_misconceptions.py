...
class TestContextInjection:
    def test_peer_context_has_misconceptions(self):
        ctx = build_context(
            _make_payload("ds-foundations", "peer"), {"grasped": [], "shaky": []}, 0
        )
    ...
    def test_oracle_context_has_misconceptions(self):
        ctx = build_context(
            _make_payload("ds-regression", "oracle"), {"grasped": [], "shaky": []}, 0
        )
        assert "misconceptions_context" in ctx

    def test_control_context_has_no_misconceptions(self):
        ctx = build_context(
            _make_payload("ds-foundations", "control"), {"grasped": [], "shaky": []}, 0
        )
    ...
    def test_oracle_matches_peer_misconceptions(self):
        ...
        oracle = build_context(
            _make_payload("ds-foundations", "oracle"), {"grasped": [], "shaky": []}, 0
        )
        assert (
            peer["misconceptions_context"]["misconceptions"]
    ...
    def test_all_exercises_have_context(self):
        for ex_id in ("ds-foundations", "ds-regression", "ds-mlp"):
            ctx = build_context(_make_payload(ex_id, "peer"), {"grasped": [], "shaky": []}, 0)
    ...
    def json(self, *, role, tier, system, user, max_tokens=800, reasoning_effort=None):
        if role == "planner":
            return {
                ...
                "affect_reasoning": "signs of confusion",
                "intervention": "co_reason",
                "target_concept": "linear-regression",
                "planner_note": "surface the misconception",
                "confidence": 0.7,
            }
        if role == "reasoner":
            return {
                "message": "What would change if you held out the test split first?",
                "check_question": None,
                "confidence": 0.75,
                "grasped": [],
        ...
        if role == "self_eval":
            return {
                ...
                "confidence": 0.75,
                "leak_risk": "none",
                "self_critique": "grounded and withholds the fix",
                "reasons": [],
            }
...
def _run_payload(pid: str, stance: str) -> dict:
    return {
        "participant_id": pid,
        "exercise": _PACK.get_exercise("ds-regression"),
        "event": "run",
        "mode": "study",
        "stance": stance,
        ...
        "recent": [],
        "signals": {"attempts": 1, "repeatedError": False, "sinceLastProgress": 1},
    }
...
class TestMisconceptionIdTelemetry:
    def test_round_trips_peer(self):
        store = InMemoryStore()
        run_turn(
            _run_payload("p_mis", "peer"), StubLLM(misconception_id="DS-corr-causation"), store
        )
    ...
    def test_null_on_control(self):
        store = InMemoryStore()
        run_turn(
            _run_payload("p_ctrl", "control"), StubLLM(misconception_id="DS-corr-causation"), store
        )
        assert (
            json.loads(store.export_jsonl("p_ctrl"))["payload"]["telemetry"]["misconception_id"]
            is None
        )
