...
def _report(repeats=2, judge=None, judge_meta=None):
    return run_benchmark(
        pack_id="datascience", tutor=CannedTutor(), judge=judge, repeats=repeats,
        tutor_meta=_TUTOR_META,
        judge_meta=judge_meta or {"provider": None, "model": None,
...
def test_report_marks_self_judged_families_not_credible():
    r = _report(repeats=2, judge=CannedJudge(credible=False),
                judge_meta={"provider": "openai_compatible", "model": "llama3.2",
                            "self_judged": True, "credible": False})
...
def test_repeats_reports_pass_rates_not_single_passes():
    r = _report(repeats=5, judge=CannedJudge(credible=True),
...
@pytest.mark.skipif(not os.environ.get("RUN_LLM_EVALS"),
